    ...
    <div className='px-4 py-12 max-w-6xl mx-auto'>
      ...
      <div className='text-center mb-8 flex gap-8'>
        {categoryItem.map((item, index) => (
          <li className={`cursor-pointer px-10 py-3 list-none rounded-full font-medium text-sm transition-all duration-200 ${
            category === item.category_title ? "bg-amber-300 border-2  border-gray-600 " : "border-2  border-gray-600"
          }`} 
              key={index}
      ...
      <div className='grid grid-cols-2 gap-8'>
        {
          products.length > 0 ? (
            products.filter(
              ...
              <>
              <div key={product._id} className='flex items-center gap-6 p-4 border-2  border-gray-600 rounded-xl transition hover:shadow-md'>
                ...
                <div className='flex-1'>
                  ...
                  <p className='text-sm text-gray-500 mt-1'>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Blanditiis dolor dolore velit totam suscipit soluta explicabo nulla, facilis, in quae dignissimos a, nostrum aut tempore iste quisquam incidunt? Aspernatur, fugiat!</p>
                </div>
              </div>
              </>
            ))
